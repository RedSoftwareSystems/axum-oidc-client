...
pub async fn build_cache(args: &Args) -> Arc<dyn AuthCache + Send + Sync> {
    use axum_oidc_client::authentication::moka::{config::TwoTierCacheConfig, TwoTierAuthCache};
    use axum_oidc_client::authentication::redis;
...
pub async fn build_cache(args: &Args) -> Arc<dyn AuthCache + Send + Sync> {
    use axum_oidc_client::authentication::sql_cache::{SqlAuthCache, SqlCacheConfig};
