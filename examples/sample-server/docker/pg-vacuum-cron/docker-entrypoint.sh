#!/bin/sh
# docker-entrypoint.sh
# ──────────────────────────────────────────────────────────────────────────────
# Entrypoint for the pg-vacuum-cron container.
#
# Responsibilities:
#   1. Write ~cronuser/.pgpass so psql can connect without a password on the
#      command line (credentials never appear in process args or logs).
#   2. Wait for PostgreSQL to accept connections (up to 60 s).
#   3. Write /etc/crontabs/cronuser with the VACUUM_SCHEDULE cron expression.
#   4. Start BusyBox crond in the foreground so the container stays alive and
#      Docker can manage its lifecycle cleanly.
#
# Why crond runs as root / why su-exec is used
# ─────────────────────────────────────────────
# BusyBox crond must be started as root — it calls setuid/setgid to switch to
# the crontab owner before executing each job.  The actual job (vacuum.sh) is
# invoked via `su-exec cronuser` so the psql process never runs with elevated
# privileges.  Credentials are written to cronuser's home (/home/cronuser) so
# they are readable by the dropped-privilege process, not by root only.
#
# Environment variables (injected by docker-compose):
#   POSTGRES_USER      – database user
#   POSTGRES_PASSWORD  – database password (written to ~cronuser/.pgpass)
#   POSTGRES_DB        – database name
#   PGHOST             – PostgreSQL hostname
#   PGPORT             – PostgreSQL port          (default: 5432)
#   VACUUM_SCHEDULE    – 5-field cron expression  (default: "0 0 * * *")
# ──────────────────────────────────────────────────────────────────────────────

set -eu

# ── Defaults ──────────────────────────────────────────────────────────────────
PGPORT="${PGPORT:-5432}"
VACUUM_SCHEDULE="${VACUUM_SCHEDULE:-0 0 * * *}"

CRONUSER_HOME="$(getent passwd cronuser | cut -d: -f6)"

# ── .pgpass — passwordless psql ───────────────────────────────────────────────
# Written to cronuser's home so the su-exec'd process can read it.
# Format: hostname:port:database:username:password
PGPASS_FILE="${CRONUSER_HOME}/.pgpass"
printf '%s:%s:%s:%s:%s\n' \
    "${PGHOST}" \
    "${PGPORT}" \
    "${POSTGRES_DB}" \
    "${POSTGRES_USER}" \
    "${POSTGRES_PASSWORD}" \
    > "${PGPASS_FILE}"
chown cronuser:cronuser "${PGPASS_FILE}"
chmod 600 "${PGPASS_FILE}"

# ── Wait for PostgreSQL to be ready ───────────────────────────────────────────
echo "[pg-vacuum-cron] Waiting for PostgreSQL at ${PGHOST}:${PGPORT} ..."
RETRIES=30
until pg_isready \
        --host="${PGHOST}" \
        --port="${PGPORT}" \
        --username="${POSTGRES_USER}" \
        --dbname="${POSTGRES_DB}" \
        --quiet \
      || [ "${RETRIES}" -eq 0 ]; do
    RETRIES=$((RETRIES - 1))
    echo "[pg-vacuum-cron] PostgreSQL not ready yet — retrying in 2 s (${RETRIES} retries left) ..."
    sleep 2
done

if [ "${RETRIES}" -eq 0 ]; then
    echo "[pg-vacuum-cron] ERROR: PostgreSQL did not become ready in time. Exiting."
    exit 1
fi

echo "[pg-vacuum-cron] PostgreSQL is ready."

# ── Write crontab ─────────────────────────────────────────────────────────────
# /etc/crontabs/ is owned by root and writable by root — no permission issues.
# The job is prefixed with `su-exec cronuser` so vacuum.sh runs unprivileged.
# crond executes jobs with a minimal environment; HOME must be set explicitly
# so psql can find ~cronuser/.pgpass.
CRONTAB_FILE="/etc/crontabs/cronuser"

cat > "${CRONTAB_FILE}" <<EOF
# VACUUM ANALYZE oidc_cache — schedule: ${VACUUM_SCHEDULE}
# Generated automatically by docker-entrypoint.sh at container startup.
# To change the schedule set VACUUM_SCHEDULE in docker-compose.postgres.yml and restart.
${VACUUM_SCHEDULE} HOME=${CRONUSER_HOME} su-exec cronuser /usr/local/bin/vacuum.sh >> /proc/1/fd/1 2>> /proc/1/fd/2
EOF

chmod 600 "${CRONTAB_FILE}"

echo "[pg-vacuum-cron] Crontab written to ${CRONTAB_FILE}:"
echo "  ${VACUUM_SCHEDULE}  su-exec cronuser /usr/local/bin/vacuum.sh"

# ── Start crond in foreground ─────────────────────────────────────────────────
# -f  run in foreground (do not daemonise — Docker needs the process alive)
# -l 8 log level 8 = debug (captures job start/stop in container logs)
# -L /dev/stdout redirects crond's own log to stdout so `docker logs` shows it
echo "[pg-vacuum-cron] Starting crond (schedule: '${VACUUM_SCHEDULE}') ..."
exec crond -f -l 8 -L /dev/stdout
