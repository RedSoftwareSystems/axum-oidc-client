...
- Schedule is controlled by `VACUUM_SCHEDULE` (default: `0 0 * * *` — every
  day at **midnight UTC**).
- Mounts the same `sqlite_data` volume as the www-server so it operates on
  the live database file.
- The database file is created by the www-server on first connection.  If
...
| `GET /tokeninfo`                  | Token info page (shows session tokens)                | Yes           |
| `GET /resources`                  | Resources page (JS fetch of API endpoints)            | Yes           |
| `GET /api/me`                     | Reverse proxy → `{api_server}/me`                     | Yes           |
| `GET /api/protected/resource1`    | Reverse proxy → `{api_server}/protected/resource1`    | Yes           |
| `GET /api/protected/resource2`    | Reverse proxy → `{api_server}/protected/resource2`    | Yes           |
...
5. **Redirected back:** See personalized home page
6. **Visit token info:** http://localhost:8080/tokeninfo
7. **Visit resources page:** http://localhost:8080/resources (calls API endpoints via JS fetch)
8. **Click logout:** Session cleared, redirected home
...
- [ENV Files Guide](ENV_FILES_GUIDE.md) — Provider env-file reference
