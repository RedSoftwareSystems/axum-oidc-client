...
//! |-------------|-----------|--------------|------------------------------------------------|
//! | `true`      | `Some(_)` | **Two-tier** | L1 first, L2 on miss, write-through (all ops) |
...
//!
//! ## Code verifier cache-aside pattern (two-tier mode)
//!
//! Code verifiers follow the same cache-aside pattern as auth sessions.
//! They are short-lived (TTL ≈ 60 s), but storing them in L2 ensures
//! correctness in multi-process deployments where the callback may be
//! handled by a different instance than the one that initiated the flow.
//!
...
impl AuthCache for TwoTierAuthCache {
    ...
    fn get_code_verifier(
        ...
        Box::pin(async move {
            ...
            if let Some(l1) = &self.l1
                && let Some(L1Entry::CodeVerifier(v)) = l1.get(&key).await
            {
                return Ok(Some(v));
            }

            // ② L2 lookup (if present).
            // No L1 population on hit: code verifiers are single-use and will
            // be invalidated immediately after retrieval, so promoting the
            // value to L1 would be wasted work.
    ...
    fn get_auth_session(
        ...
        Box::pin(async move {
            // ① Check L1
            if let Some(l1) = &self.l1
                && let Some(L1Entry::AuthSession(s)) = l1.get(&key).await
            {
                return Ok(Some(s));
            }
            ...
            if let Some(l2) = &self.l2 {
                ...
                if let (Some(l1), Some(s)) = (&self.l1, &result) {
...
mod tests {
    ...
    //
    // In two-tier mode code verifiers follow the same cache-aside pattern as
    // auth sessions: L2 is written first (source of truth), L1 is written
    // afterwards; reads check L1 first and fall back to L2, populating L1 on hit.
    ...
    /// A code verifier written directly into L2 must be visible through the
    /// two-tier cache (served via L2 fallback), but must NOT be promoted to L1
    /// because code verifiers are single-use and will be invalidated immediately
    /// after retrieval — caching them in L1 would be wasted work.
    #[tokio::test]
    async fn test_two_tier_code_verifier_l2_no_l1_promotion() {
        let stub = StubL2::new();

        // Seed L2 directly (simulates a write from another process instance)
        stub.set_code_verifier("s2", "v2").await.unwrap();

        let cache = make_two_tier(&stub);

        // L1 is empty; the cache should fall back to L2 and return the value.
        assert_eq!(
            cache.get_code_verifier("s2").await.unwrap(),
            Some("v2".to_string()),
            "two-tier cache must fall back to L2 for code verifiers on L1 miss"
        );

        // L1 must NOT be populated — no promotion for single-use values.
        let l1 = cache.l1.as_ref().unwrap();
        assert!(
            l1.get(&cv_key("s2")).await.is_none(),
            "code verifier must not be promoted to L1 on an L2 hit"
        );
    }
